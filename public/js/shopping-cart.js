    ...
    if (userData.length !== 1) {
        if (cartGuest.length === 0) {
            ...
            for (i = 0; i < cartGuest.length; i++) {
                if (cartGuest[i].shoes_quantity == 1) {
            ...
            <tr>
                                <td>
                                    <div class="d-flex gap-3 align-items-center ">
                                        <div>
                                            <img class="cart-img" src="${cartGuest[i].shoes_img}" alt="cart-img">
                                        </div>
                                        <div>
                                            ${cartGuest[i].shoes_name}
                                        </div>
                                        <div>
                                            Size ${cartGuest[i].shoes_size}
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="current-price">${cartGuest[i].shoes_price}.00</span>
                                </td>
                                <td>
                                    <div class="quantity-button">
                                        <span class="decrease ${disabledClass}">-</span>
                                        <input class="quantity-input" type="number" name="quantity" id="quantity" value="${cartGuest[i].shoes_quantity}" min="1" max="100">
                                        <span class="increase">+</span>
                                    </div>
                                </td>
                                <td>
                                    <span class="total-price">$${cartGuest[i].shoes_price.replace("$", "") * cartGuest[i].shoes_quantity}.00</span>
                                </td>
                                <td>
...
function checkOutButton() {
    if (userData.length !== 1) {
        ...
        for (i = 0; i < cartGuest.length; i++) {
            cartGuest[i].shoes_quantity = $(".quantity-input").eq(i).val()
        }
...
$(".remove-agree").on("click", function () {
    ...
    // Guest User
    if (userData.length !== 1) {
        cartGuest.splice(indexRemove, 1)
        localStorage.setItem("cart-guest", JSON.stringify(cartGuest));
        navHtml()
        tableCart()
    ...
    // Login user
        ...
        // Change Nubmer item on cart
        userData[0].cart.splice(indexRemove, 1)
        loginNav()

        // Renew User's cart database 
        let cartPatch = { "cart": [] };
        for (i = 0; i < userData[0].cart.length; i++) {
            userData[0].cart[i].shoes_quantity = $(".quantity-input").eq(i).val()
            cartPatch.cart.push(userData[0].cart[i]);
